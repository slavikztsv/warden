...
| `broker/audit.py` | Append-only hash-chained decision log. The active segment is `/data/audit.jsonl`; at `[audit].segment_bytes` it closes into `/data/audit-<last seq>.jsonl` and the new segment's first record anchors to it by name and by hash, so one chain spans every file | Trusted record | Tool API returns 503 and **nothing executes** |
